...
SETTING_HELP = {
    ...
    # ── 10. Scheduling ────────────────────────────────────────────────────
    '10.01': {'label': 'Shotgun Start', 'text': "Changes how tee times/starting holes work for this season. Off (default): every group can have its own staggered tee time, all starting from the same hole. On: every group starts at the same time, but at a different hole around the course — set up a reusable default (one hole per group, for the front 9 and the back 9) on the Tee Time Template page, and it auto-fills every week's schedule instead of being re-entered each time. A specific week can still be hand-edited afterward. Doesn't apply to a season running Open Schedule below."},
    '10.02': {'label': 'Open Schedule', 'text': "Turns this season into a freeform, member-driven schedule instead of an admin-generated round-robin. Any member can create a brand-new match at any time — pick an opponent and a date, and it's added straight to the schedule, no approval queue, no guarantee of a complete round-robin, and the same opponents can play more than once. Works for both 2-player teams and solo (individual) rosters — a member can only be offered opponents of the same format (solo vs. solo, or team vs. team). Admins can still create/edit/remove matches as usual. Turning this on hides Generate Schedule, Add Week, Shotgun Start, weekly Skins & Contests, and Availability polls for this season, since those all assume a synchronized league-wide week that a freeform schedule doesn't have."},
}
