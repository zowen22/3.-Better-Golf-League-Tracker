...
**@user's explicit instruction: the 3 named metrics (league counts, API pull counts/status) are the floor, not the ceiling.** Fable has latitude to include anything else it judges worth surfacing on a platform-health dashboard, using its own judgment about what a site admin/operator would actually want to see at a glance — this isn't meant to be a rigid, closed spec. Some candidates already visible from this investigation, offered as a starting point (not a requirement): seasons-per-league or total active seasons; players/teams counts platform-wide; error-rate or 4xx/5xx breakdown from `api_request_log.response_code` (already captured, currently unused for anything); most-recently-created leagues (onboarding-funnel visibility); any leagues with `active = 0` (churn visibility). Fable should feel free to add, drop, or substitute based on what's genuinely useful and cheap to compute from existing data — the goal is a useful dashboard, not literal compliance with the 3-item list above.
