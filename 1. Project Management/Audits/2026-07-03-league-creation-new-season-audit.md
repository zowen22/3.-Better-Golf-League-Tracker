...
**Status:** Bugs — Complete. Feature recommendations — Open, needs @user priority call.
...
4. Minor: add a DB-level `UNIQUE` constraint on `league_name` (or an app-level lock/transaction) and a password-strength requirement matching `register()`'s ≥6-char rule, applied to league admin/member passwords too.
