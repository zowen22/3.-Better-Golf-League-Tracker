...
**Status:** All 4 items resolved 2026-07-10. (1) `ab_designation_method` removed. (2) Match Play points wired + `match_play_tie_points` added. (3) Skins `handicap_percent` — after the correction below revealed the real build cost, @user chose option (b): **dropped the column entirely** (`migrations/drop_skins_config_handicap_percent.sql`) rather than building the read-only point-in-time helper. (4) WHS differential — implemented via new `_compute_differential()` shared helper. All 4 validated against real dev Postgres.
...
**Resolution 2026-07-10 — @user chose (b): drop the setting.** No admin UI ever exposed this field at all (re-confirmed while removing it — `templates/skins/index.html` has no form field for it, and the `skins_config` INSERT/UPDATE in `skins.py` never included the column), so removal was a pure schema-only change: dropped from `schema_postgres.sql`/`init_db.py`, `migrations/drop_skins_config_handicap_percent.sql` for existing dev/prod DBs. Validated against real dev Postgres: migration applied cleanly, skins pages still render correctly with the column gone.
