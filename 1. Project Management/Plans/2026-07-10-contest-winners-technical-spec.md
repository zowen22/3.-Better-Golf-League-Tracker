...
*Status: `Ready to build` — one nav-placement question still open (see below), otherwise fully grounded.*
...
## 1. Unified Contest Winners report

**Route:** `GET /contests/winners` (standalone read-only report — kept separate from `admin_edit.html`'s per-contest editing view, per the decision doc's Approach A).
...
## 2. Per-player $ won across contests

Same query as #1, grouped instead of listed:
...
## Nav placement — still open

The decision doc's own open question was never resolved: does this live as a new tab on the existing Contests page, or under Stats & Records? Recommend **Contests page, new "All Results" tab** — these are fundamentally contest data, and a member looking for "who won what" is more likely to look under Contests than Stats. Flagging for a quick confirm before templates are wired into the nav, not blocking the backend build.

## Effort: S–M overall. No schema changes needed for #1/#2/#4 (reuse existing tables); #3 reuses existing calculation code, no new query logic. One new route file section (or additions to `contests.py`), 2-3 new templates depending on whether #1-#2 share one page (recommend they do — same underlying data, just grouped differently, one page with a toggle) or split.
...
Ready to build once the nav-placement question above is answered.
